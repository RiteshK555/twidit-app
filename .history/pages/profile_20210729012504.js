...
const Profile = ({serverPosts,session,serverBio}) => {
    ...
    function handleTwidit(e) {
        ...
        function handleTwiditClient(){
            ...
            // setPosts([{data:fakeTwidit.data,name:session.user.name,likes:0},...posts])
            async function handleIdClient(){
                const snapshot1= await db.collection("posts").where("data", "==",session.user.name).get();
                snapshot1.forEach((doc)=>{
                    setPosts({data:doc.data().data,likes:doc.data().likes,id:doc.id,name:doc.data().name});
                })
                .then(()=>{
                    setPosts
                });
