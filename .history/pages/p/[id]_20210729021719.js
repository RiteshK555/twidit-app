...
const Id = ({post,params,session})=> {
    ...
    function handleSubmitComment(e){
        ...
        async function handleSubmitCommentServer(){
            await db.collection("posts").doc(params.id).collection("comments")
            .add({
    ...
    return ( 
        <>
            ...
            {
                <div style={{marginLeft:411,marginRight:300,marginTop:20,marginBottom:20}}>
            <Card style={{display: 'flex',justifyContent: 'center'}}>
                <Card.Bodystyle={{cursor:"pointer"}}>
                hi
                </Card.Bodystyle=>
                <Button  style={{marginLeft:500,marginBottom:12}} size="sm" variant="danger">
                     delete
                </Button>
            </Card>
