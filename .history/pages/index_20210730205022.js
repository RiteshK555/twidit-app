...
export default function Page({AllPosts,session}) {
  ...
  return <>
    ...
    {
      AllPosts.length===0?<>
    <div style={{display:'flex', justifyContent: 'center',margin:123}}>
      Follow more people to get feed in your timeline.   
    </div>
      </>
      :<></>
    }
        ...
        { 
            AllPosts.map((post) => <Post name={post.name} boolButton={false} id={post.id} likes={post.likes} postsData={post.data} />)
        }
    </>
    }
  </>
