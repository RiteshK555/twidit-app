    ...
    function handleTwidit(e) {
        ...
        function handleTwiditClient(){
            ...
            async function handleIdClient(){
                ...
                const snapshot2=await db.collection("users").doc(doc.id).collection("posts").where("data","==",fakeTwidit.data).get();
                snapshot2.forEach((doc)=>{
                    setPosts([...posts,{data:fakeTwidit.data,likes:0,id:doc.id}])
                });
            }
