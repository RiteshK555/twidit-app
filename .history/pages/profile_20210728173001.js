...
export async function getServerSideProps(ctx) {
  ...
  var serverBio="";
  var serverPosts=[];
         const db1= await db.collection("users").where("name","==",session.user.name)
               .get()
               .then((querySnapshot) => {
                ...
                async ()=>{
                await db.collection("users").doc(doc.id).collection("posts")
                .get()
                .then((querySnapshot) =>{
                    querySnapshot.forEach((doc)=>{
  ...
  return {
      props: {
          session,serverBio,serverPosts
        },
    }
}
const Profile = ({session,serverBio,serverPosts}) => {
    // console.log(serverPosts);
    // console.log(serverBio);
