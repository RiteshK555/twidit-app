...
const Profile = ({serverPosts,session,serverBio,name}) => {
    ...
    return (
            ...
            {session.user.name===serverPosts[0].name?<Button size="sm" onClick={handleEdit}>Edit</Button>:<></>}
            </div>
            </>
            }
        ...
        { 
            posts.map((post) => <Post boolButton={session.user.name===serverPosts} id={post.id} handleAsyncLikePost={handleAsyncLikePost} likes={post.likes} postsData={post.data} handleDelete={handleDeletePost}/>)
        }
        </>
