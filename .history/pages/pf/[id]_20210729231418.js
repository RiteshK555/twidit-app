    ...
    function handleFollow(){
        ...
        async function handleFollowServer(){
            ...
            if(follow==="warning"){
                ...
                var docIDD;
                const znapshot=await db.collection("users").where("name","==",session.user.name).get();
                znapshot.forEach((doc)=>{
                    if(doc.name===session.user.name) {
                        docIDD=doc.id;
                    }
                })
                console.log("al;jsksd;jlksdf");
                const snap=await db.collection("users").doc(name1).collection("following").where("name","==",session.user.name).get();
                // snap.forEach((doc)=>{

                // })
                // console.log(snap);
                await db.collection("users").doc(docIDD).collection("followers").doc(idHandleFollowServer).delete();
