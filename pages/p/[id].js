            ...
            </div>
            {
                comments.map((comment)=><>
                        ...
                        <Card style={{display: 'flex',justifyContent: 'center'}}>
                            <Card.Title style={{margin:12}}>{comment.name}</Card.Title>
                        <Card.Body>
                        {comment.comment}
                        </Card.Body>
                        <div style={{display: 'flex',justifyContent: 'center',paddingLeft:445,marginBottom:12}}>
                            {comment.name===session.user.name ?<Button id={comment.id} onClick={handleDelete} size="sm" variant="danger">delete</Button>:<></>}
                        </div>
                        </Card>
                        </div>  
                    </>
