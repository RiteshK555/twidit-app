...
const Profile = ({serverPosts,session,serverBio,name,isFollowing}) => {
    ...
    function handleFollow(){
        ...
        async function handleFollowServer(){
            ...
            var idHandleFollowServer;
                ...
                var sessionUserId;
                const snapshotSessionUserId=await db.collection("users").where("name","==",session.user.name).get()
                snapshotSessionUserId.forEach((doc)=>{
                    if(doc.data().name===session.user.name){
            ...
            if(follow==="warning"){
                ...
                await db.collection("users").doc(sessionUserId).collection("following")
                .add({
                    name:name,
                })
            }else{
                var delFollowingId;
                const snap=await db.collection("users").doc(sessionUserId).collection("following").where("name","==",name).get();
                snap.forEach((doc)=>{
                    if(doc.data().name===name){
                        delFollowingId=doc.id;
                    }
                })
                await db.collection("users").doc(sessionUserId).collection("following").doc(delFollowingId).delete();
    ...
    <></>
        ...
        { 
            posts.map((post) => <Post name={post.name} postsData={post.data} likes={post.likes} boolButton={session.user.name===name} id={post.id}/>)
        }
        </>
